...
if DEBUG:
    # Development mode - use standard configuration
    TEMPLATES = [
        {
            "BACKEND": "django.template.backends.django.DjangoTemplates",
            "DIRS": [
                BASE_DIR / "templates",
                BASE_DIR / "trialsfinder" / "build",  # Also check build directory
                BASE_DIR / "trialsfinder" / "dist",   # Check dist directory
            ],
            "APP_DIRS": True,
            "OPTIONS": {
                "context_processors": [
    ...
    # Production mode - use cached loader
    TEMPLATES = [
        {
            "BACKEND": "django.template.backends.django.DjangoTemplates",
            "DIRS": [
                BASE_DIR / "templates",
                BASE_DIR / "trialsfinder" / "build",
...
# Only add SQLite optimizations if not running tests
...
CACHES = {
    "default": {
        ...
        "KEY_PREFIX": "trialssfinder",
        "TIMEOUT": 300,
    }
}

# Try locmem if Redis not available
try:
    import redis
...
# Clear and rebuild STATICFILES_DIRS
STATICFILES_DIRS = []

# Look for React build in multiple possible locations
react_locations = [
    BASE_DIR / "trialsfinder" / "build",
    BASE_DIR / "trialsfinder" / "dist",
]

# Find which directory exists and has static files
REACT_BUILD_DIR = None
for location in react_locations:
    if location.exists() and (location / "static").exists():
        REACT_BUILD_DIR = location
        STATICFILES_DIRS.append(location / "static")
        break

# If no React build found, just use the expected location
...
# CORS configuration - Updated to include port 8080 if needed
...
if DEBUG:
    ...
    # Ensure all common development ports are allowed
    dev_origins = [
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://localhost:8000",
...
CSRF_TRUSTED_ORIGINS = ["http://localhost:3000", "http://127.0.0.1:3000", "http://localhost:8000", "http://localhost:8080"] if DEBUG else CORS_ALLOWED_ORIGINS
CSRF_FAILURE_VIEW = "apps.core.views.csrf_failure"

# Content Security Policy - Updated to handle port 8080
if DEBUG:
